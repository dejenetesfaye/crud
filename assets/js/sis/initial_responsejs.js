 $(document).ready(function () {
    ...
    $(document).on("click", ".editInitTickets", function () {
        ...
        var note3 = $(this).data('note3');
        var status = $(this).data('status');
        //var alert ="are you sure you want to delete "+name +" Asset ?";
        //$(".modal-body #editInitTicket").html(alert);
    ...
    function EditTicket()
    {
        ...
        } else if (!Note_3)
        {
            $('#confirm_modal').modal('show');
            $('#confirm_title').html("Information");
            $('#msg_conf').html("Please Fill Note3 Field!");
            $('#msg_conf').removeClass("alert-info");
            $('#msg_conf').addClass("alert-danger");
        ...
        } else if (isValidEmailAddress(email) && (!subject || !body))
        {
            ...
            $.ajax({
                url: '../Tickets/Add_final_response',
                method: 'POST',
                dataType: 'json',
                data: {title: ticketId, uname: Res_user_, note: Note_, note2: Note_2, note3: Note_3, priority: priority_, createBy: createBy_, status: status_, email: email, subject: subject, body: body},
                //jsonData: id,
